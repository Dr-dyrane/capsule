...
export function toUploadErrorMessage(error: unknown) {
...
export async function uploadNoteFromFormData(formData: FormData) {
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        const {
          data: { user: cleanupUser },
        } = await cleanupSupabase.auth.getUser()
